  ...
  >
    ...
    <div class="table-content">
      <table>
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.id" :id="row.id">
            <td
              v-for="(value, rowidx, n) in row"
              :key="rowidx"
              :style="dataColumns[n].style"
            >
              <div v-if="value">
                <CheckBox
                  v-if="!dataColumns[n].style"
                  :id="row.id + rowidx"
                  :value="value"
                  :name="row.name"
                  :complement="rowidx"
                  :event="eventAddChecked"
                  :disabled="!value"
...
@Options({
  ...
  methods: {
    ...
    eventAddChecked(checkGrid: checkBox_type) {
      console.log(checkGrid);
      this.eventlistChecked(checkGrid);
    },
  },
  mounted() {
